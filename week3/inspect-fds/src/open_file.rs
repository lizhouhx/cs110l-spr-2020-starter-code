...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let fd_path = format!("/proc/{}/fd/{}",fd, pid);
        let path_buf = fs::read_link(fd_path).ok()?;
        let filename = OpenFile::path_to_name(path_buf.to_str()?);

        let fdinfo_path = format!("/proc/{}/fdinfo/{}",fd, pid);
        let fdinfo = read_to_string(fdinfo_path).ok()?;
